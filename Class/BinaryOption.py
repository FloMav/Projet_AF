from Class.VanillaOption import VanillaOption
from Class.BlackScholes import BlackScholes
from Class.Smile import Smile
import pandas as pd
...
class BinaryOption(BlackScholes):
    ...
    Replication:
    Call spread of Vanilla options with overhedge: K2 = K and K1 = KM (strike modofied)

    Smile:
    Adjustement of the volatility throughout the class according to strike and maturity
    ...
    def rep_option_km(self, b) -> VanillaOption:
        if b == "Bear":
    ...
    def recorder(self, date: str) -> None:
        li = [self.typ,
                self.spot,
                self.strike,
                self.rate,
    ...
    def setter(self, inp: tuple, date: str) -> float:
        self.spot = inp[0]
        self.rate = inp[1]
        self.dividend = inp[2]
        self.maturity -= 1
        if not self.__smile_active:
            ...
            #self.__volatility_km_bear = self.smile(date, self.__strike_km_bear) ######## COMMENT/UNCOMMENT
            #self.__volatility_km_bull = self.smile(date, self.__strike_km_bull) ######## COMMENT/UNCOMMENT
            self.__volatility_km_bear = self.volatility ######## COMMENT/UNCOMMENT
            self.__volatility_km_bull = self.volatility ######## COMMENT/UNCOMMENT
        self.recorder(date)
...
####################### SMILE
    ...
    def smile(self, date, strike):
        smile = Smile(self.spot, self.maturity, date)
